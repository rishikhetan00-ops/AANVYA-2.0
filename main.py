...
# ── Windows: Ensure PyQt6 DLLs are loaded cleanly on Anaconda / multi-Qt environments
if _platform.system() == "Windows":
    try:
        import ctypes
        from pathlib import Path
        _pyqt6_bin = Path(_sys.executable).parent / "Lib" / "site-packages" / "PyQt6" / "Qt6" / "bin"
        if _pyqt6_bin.exists():
            ctypes.windll.kernel32.SetDllDirectoryW(str(_pyqt6_bin))
            if hasattr(_os, "add_dll_directory"):
                try:
                    _os.add_dll_directory(str(_pyqt6_bin))
                except Exception:
                    pass
            for _dll in _pyqt6_bin.glob("*.dll"):
                try:
                    ctypes.windll.kernel32.LoadLibraryExW(str(_dll), None, 0x00000008)
                except Exception:
                    pass
    except Exception:
        pass
